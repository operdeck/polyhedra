...
```{r, include=F}
clear3d()
drawPoly(quasiarchimedeans, start = c(1, 1, 1), delta = c(2, -0.5, 0))
...
NB Truncate transformation doesnt seem to work well for the star polyhedra.
Also, some work to do to make the Truncate transform more generally applicable e.g. simply truncate(quasi(cube))
...
```{r, include=F}
clear3d()
drawPoly(truncatearchimedeans, start = c(1, 1, 1), delta = c(2, -0.5, 0))
snapshot3d("snapshots/truncatearchimedeans.png", fmt = "png", top = TRUE )
```

![Truncate Archimedean solids](snapshots/truncatearchimedeans.png)
