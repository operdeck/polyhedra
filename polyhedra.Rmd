...
Every vertex becomes a face like in the rhombic transformation, but part of the original edges are kept, so in effect the original faces are doubled in number of sides.

Snub transformation doesnt seem to work well for the star polyhedra.

```{r}
snubarchimedeans <- lapply(Platonics, snub)
```

```{r, include=F}
clear3d()
drawPoly(snubarchimedeans, start = c(1, 1, 1), delta = c(2, -0.5, 0))
snapshot3d("snapshots/snubarchimedeans.png", fmt = "png", top = TRUE )
```

![Snub Archimedean solids](snapshots/snubarchimedeans.png)
...
For the tetrahedron and octahedron nothing new emerges but from the next two a more interesting family of polyhedra arises although unfortunately no new polyhedra relative to the ones shown before.
