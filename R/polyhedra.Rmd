...
library(colorspace)
drawPolyWidget(Platonics, start = c(1, 1, 1), delta = c(2, 0, 0), label = "Platonic solids", colorProvider = qualitative_hcl)
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
drawPolyWidget(KeplerPoinsots, start = c(1, 1, 1), delta = c(2, -0.5, 0), label = "Kepler Poinsot solids")
```
...
combis <- lapply(Regulars[seq_along(Regulars)%%2==1], function(p) { return(compose(p, dual(p))) })
```

```{r, echo=FALSE, warning=FALSE, message=FALSE}
drawPolyWidget(combis, start = c(1, 1, 1), delta = c(2, -0.5, 0), label = "Compositions with dual",
...
quasiarchimedeans <- lapply(Regulars[seq_along(Regulars)%%2==1], quasi)
```

```{r, echo=FALSE, warning=FALSE, message=FALSE}
drawPolyWidget(quasiarchimedeans, start = c(1, 1, 1), delta = c(2, -0.5, 0), label="Quasi regular solids")
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
drawPolyWidget(truncatearchimedeans, start = c(1, 1, 1), delta = c(2, -0.5, 0), label="Truncated regular solids", colorProvider = heat.colors)
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
drawPolyWidget(truncatedstars, start = c(1, 1, 1), delta = c(2, -0.5, 0), label="Truncated Kepler Poinsot solids")
```
...
```{r}
rhombicregulars <- lapply(c(Platonics[seq_along(Platonics)%%2==1], 
                            list(greatDodecahedron, greatIcosahedron)), rhombic)
# NB rhombic(smallStellatedDodecahedron) fails currently
```

```{r, echo=FALSE, warning=FALSE, message=FALSE}
drawPolyWidget(rhombicregulars, start = c(1, 1, 1), delta = c(2, -0.5, 0), label="Rhombic transformation",
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
drawPolyWidget(irregulars, start = c(1, 1, 1), delta = c(2, -0.5, 0), label="Some irregular solids")
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
drawPolyWidget(list(compound5tetrahedra, dual(compound5tetrahedra)), 
         start = c(1, 1, 1), delta = c(2, -0.5, 0), label = "Compound of 5 Tetrahedra + dual")
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
drawPolyWidget(list(compound5octahedra, compound5truncatedTetrahedra),
         start = c(1, 1, 1), delta = c(2, -0.5, 0), label = "Transformations of 5 Tetrahedra")
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
drawPolyWidget(compound10tetrahedra)
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
drawPolyWidget(moreDodecahedronStellations, start = c(1, 1, 1), delta = c(2, -0.5, 0), label = "Stellations in Dodecahedron family")
```
...
drawPolyWidget(variationOnQuasiDodecahedron)
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
drawPolyWidget(cube_discoveries$polyhedra, label = "Discoveries based on cube")
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
drawPolyWidget(dodecahedron_discoveries$polyhedra, label = "Discoveries based on dodecahedron")
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
drawPolyWidget(icosahedron_discoveries$polyhedra, label = "Discoveries based on icosahedron")
```
